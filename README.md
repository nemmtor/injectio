![Injectio header](https://github.com/nemmtor/injectio/raw/main/media/header.webp)
...
```tsx
const injectSomething = inject<undefined>({
  renderFn: ({ deferred }) => (
    <Something
      onConfirm={() =>
        deferred.pipe(Deferred.succeed(undefined), Effect.runPromise)
...
const injectProfileFormDialog = inject<
  ProfileFormOutput,
  ProfileFormCancelledError
>({
  renderFn: ({ deferred }) => (
...
const injectSomeDialog = inject<undefined, never, InjectedProps>({
