...
const waitForProgress = (basicProfile: BasicProfile) =>
  Effect.gen(function* () {
    ...
    const nextStep = yield* Effect.raceFirst(apiResult, slowPath);

    return nextStep;
  }).pipe(Effect.scoped);

/**
 * More complex usecase with usage of updateProps to dynamically update the progress dialog
 */
export const startExample5 = () =>
  Effect.gen(function* () {
    ...
    yield* waitForProgress(basicProfile);
